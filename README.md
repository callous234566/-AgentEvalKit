...
Personal RAG Assistant is a local-first personal RAG knowledge base assistant built with **FastAPI + Streamlit + LangChain + Chroma + SiliconFlow + Tavily**. It focuses on a complete, maintainable AI application workflow: document ingestion, hybrid retrieval, grounded answers with citations, Agent tool routing, Web fallback, request-id diagnostics, and evaluation baselines.

Personal RAG Assistant 是一个本地优先、面向长期维护的个人 RAG 知识库助手，技术栈为 **FastAPI + Streamlit + LangChain + Chroma + SiliconFlow + Tavily**。项目重点不是简单问答原型，而是把文档入库、混合检索、引用追踪、Agent 工具调用、Web 降级、request id 排障和评测基线做成完整闭环。

![Workbench](docs/images/workbench.png)

## Contest Mode: AI Marketing Intelligence Copilot

This branch adds a focused marketing workflow on top of the existing local-first RAG assistant. It is built for media buyers who need to turn offer briefs, landing-page copy, ad examples, competitor notes, and performance notes into practical campaign work.

**What does this tool do?**

- Ingests sanitized marketing documents such as offer briefs, customer notes, landing-page copy, ad performance CSVs, and competitor angle notes.
- Uses local RAG plus optional Tavily/Web fallback to generate campaign angles, landing-page critiques, creative briefs, and test plans.
- Shows citations, source snippets, Agent evidence summaries, and request-id diagnostics so recommendations are reviewable instead of being a black box.

**Why build this tool?**

Performance marketing teams often lose time moving between messy research, scattered offer notes, landing pages, and ad testing spreadsheets. A generic chatbot can brainstorm, but a useful media-buying copilot needs to ground recommendations in the actual offer, customer language, page copy, performance notes, and market evidence. This project already had the right architecture for that: ingestion, hybrid retrieval, citations, Agent routing, Web fallback, evals, and diagnostics.

**What would I build next?**

- A lightweight competitive intelligence crawler that tracks landing-page and ad-angle changes over time.
- A creative testing dashboard that connects hypotheses to spend, CPA, CTR, CVR, and winning/losing angles.
- A compliance and claims-risk reviewer for health, finance, and affiliate offers.
- A campaign memory layer that learns which hooks, objections, and proof points worked by offer and channel.
- A hosted demo with tenant-separated workspaces and background ingestion jobs.

### Marketing Sample Data

Fictional sample files are included under `samples/marketing/`:

- `offer_brief.md`
- `target_customer_notes.md`
- `landing_page_copy.md`
- `ad_performance.csv`
- `competitor_angle_notes.md`

Upload these files into a new knowledge base, open the **营销工作台 / Marketing Workspace** tab, and run one of the built-in workflows:
...
- [Contest Mode: AI Marketing Intelligence Copilot](#contest-mode-ai-marketing-intelligence-copilot)
...
Many RAG prototypes stop at "upload a file and ask a question". This project focuses on the engineering pieces that make a personal knowledge assistant reliable enough to use, debug, and maintain: reproducible ingestion, explainable retrieval, grounded citations, Agent routing, Web fallback, request-id diagnostics, and eval baselines.

很多 RAG 原型停留在“上传文件然后问答”。这个项目更关注一个个人知识库助手真正可使用、可排障、可维护所需的工程闭环：可复现入库、可解释检索、引用来源、Agent 路由、Web 降级、request id 日志链路和评测基线。
...
- Sanitized sample documents under `zhishiku/demo_*.md`.
...
公开仓库包含源码、测试、文档、脱敏示例资料、CI 和 GitHub 协作模板；不包含真实密钥、本地 Chroma、日志、缓存、备份、真实 eval report 或私有知识库正文。

## Roadmap / 后续路线

- Docker / docker-compose for easier clean-room setup.
...
- More sanitized eval cases and sample datasets.
...
截图只展示脱敏示例内容，不包含 `.env`、密钥、本地 Chroma、日志或私有文档正文。
...
├── zhishiku/demo_*.md              # Public sanitized sample docs
